...
export const revisedBudgetConfig: FormConfig = {
    ...
    fields: [
        ...
        { label: "Budget Details:", value: "", type: "text" },
        { label: "7. Expenses (E) : As per actuals:", value: "", type: "text" },
        { label: "i. Cost of registration material:", value: "", type: "subfield", indentLevel: 1 },
...
export const mapRevisedBudgetDataToConfig = (formData: any): FormConfig => {
    const config = { ...revisedBudgetConfig };
    
    // Map form data to field values
    config.fields[0].value = formData.courseCode || "";
    config.fields[1].value = formData.courseNameAndDate || "";
    config.fields[2].value = formData.originalBudget?.toString() || "";
    config.fields[3].value = formData.revisedBudget?.toString() || "";
    ...
    // Expenses (kept for backward compatibility, but won't be rendered)
    config.fields[5].value = ""; // Section header
    config.fields[6].value = formData.expenses?.registrationMaterial?.toString() || "";
    config.fields[7].value = formData.expenses?.contingency?.toString() || "";
    config.fields[8].value = formData.expenses?.infrastructure?.toString() || "";
    config.fields[9].value = formData.expenses?.accommodation?.toString() || "";
    config.fields[10].value = formData.expenses?.transportation?.toString() || "";
    config.fields[11].value = formData.expenses?.localTravel?.toString() || "";
    config.fields[12].value = formData.expenses?.labStaff?.toString() || "";
    config.fields[13].value = formData.honorarium?.toString() || "";
    ...
    config.multilineFields![1].value = formData.reasonForExtension || "";
